...
class DefaultHTMLParser(BaseParser):
    ...
    def _extract_images(self, base_url: str, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        ...
        for img in img_tags:
            ...
            if (width and width < 32) or (height and height < 32): continue
    ...
    def _extract_date(self, soup: BeautifulSoup) -> Optional[str]:
        meta_targets = [{"property": "article:published_time"}, {"name": "pubdate"}, {"itemprop": "datePublished"}]
        for attrs in meta_targets:
            tag = soup.find("meta", attrs=attrs)
            if tag and tag.get("content"): return tag.get("content")
        return None
    ...
    def _extract_category(self, url: str, soup: BeautifulSoup) -> str:
        ...
        if parts:
            candidate = parts[0]
            if len(candidate) > 2 and candidate not in ('en', 'ja', 'v1', 'api'): return candidate
        return "general"
